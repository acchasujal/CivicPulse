...
| Escalation Agent (real send/export) | Tier 2 — not started | Approved draft, SMTP creds | solo dev | `agent_5_escalation.py`, `email_client.py`, `pdf_export.py` | `POST /escalations` | none (action layer) | High | This is the single most important live-demo proof point — test the real send well before Day 6 |
